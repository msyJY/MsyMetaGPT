...
def update_plan_from_rsp(rsp: str, current_plan: Plan):
    if not rsp or not rsp.strip():
        raise ValueError("Plan response is empty; cannot update plan.")
    try:
        rsp = json.loads(rsp)
    except json.JSONDecodeError as e:
        logger.error("Failed to parse plan response as JSON", exc_info=e)
        raise ValueError(f"Plan response is not valid JSON: {e.msg}") from e
