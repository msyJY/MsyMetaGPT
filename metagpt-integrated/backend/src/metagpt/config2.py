...
class Config(CLIParams, YamlModel):
    ...
    def default(cls):
        ...
        # 首先从环境变量中加载LLM配置
        env_llm_config = {}
        if os.getenv("METAGPT_LLM_API_TYPE"):
            env_llm_config["api_type"] = os.getenv("METAGPT_LLM_API_TYPE")
        if os.getenv("METAGPT_LLM_BASE_URL"):
            env_llm_config["base_url"] = os.getenv("METAGPT_LLM_BASE_URL")
        if os.getenv("METAGPT_LLM_API_KEY"):
            env_llm_config["api_key"] = os.getenv("METAGPT_LLM_API_KEY")
        if os.getenv("METAGPT_LLM_MODEL"):
            env_llm_config["model"] = os.getenv("METAGPT_LLM_MODEL")
        if os.getenv("METAGPT_LLM_MAX_TOKEN"):
            env_llm_config["max_token"] = int(os.getenv("METAGPT_LLM_MAX_TOKEN"))
        if os.getenv("METAGPT_LLM_TEMPERATURE"):
            env_llm_config["temperature"] = float(os.getenv("METAGPT_LLM_TEMPERATURE"))

        # 合并所有配置源：环境变量 > 配置文件 > 默认值
        dicts = [dict(os.environ)]
        dicts += [Config.read_yaml(path) for path in default_config_paths]
        
        # 如果环境变量中存在LLM配置，则将其作为最高优先级合并
        if env_llm_config:
            dicts.append({"llm": env_llm_config})

        final = merge_dict(dicts)
        
        # 确保llm字段被正确解析为LLMConfig对象
        if "llm" in final and isinstance(final["llm"], dict):
            final["llm"] = LLMConfig.model_validate(final["llm"])
