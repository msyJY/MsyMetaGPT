...
class APIRequestor:
            ...
            async def wrap_resp():
                ...
                try:
                    ...
                    try:
                        ...
                        # When a streaming request is interrupted (e.g. user stop),
                        # aiohttp's async context manager may surface
                        # ``anext(): asynchronous generator is already running``.
                        # Suppress this benign shutdown noise while keeping other errors.
                        if "asynchronous generator is already running" in str(exc):
                            logger.debug("wrap_resp context already closing: %s", exc)
                        else:
                            raise
    ...
    def request_raw(
        ...
        *,
        ...
        if not hasattr(_thread_context, "session"):
            ...
            _thread_context.session_create_time = time.time()
        try:
            # Debug: print composed request info to help diagnose endpoint/path issues
            log_info(
                "Composed sync request",
                method=method,
                url=abs_url,
                stream=stream,
    ...
    async def arequest_raw(
        ...
        *,
        ...
        try:
            # Debug: print composed async request info to help diagnose endpoint/path issues
            log_info(
                ...
                params=params,
                data_preview=(None if data is None else (data[:200] if hasattr(data, '__len__') else str(type(data)))),
            )
